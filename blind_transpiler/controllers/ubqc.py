...
class UBQC(BaseController):
    def __init__(self):
        BaseController.__init__(self)

        None
    def ubqc(self, trans_circ, keys):
        '''
        ...
        from ..translators.universalRecursiveRotationTranslator import UniversalRecursiveRotationTranslator

        # client_basis = self.client_basis 
        # server_basis = self.ubqc_server_basis
        constant_compute_space = GVF.ubqc_compute_space # defined it at a central location
        ...
        bqc_instructions.format = 'ubqc'

        translation_map = self._create_translation_map(UniversalRecursiveRotationTranslator(), format='ubqc')
        ...
        for step, data in enumerate(trans_circ.data):
            ...
            if instr.name in self.client_basis:
                ...
                gate_key_size = self.ubqc_key_map[instr.name]
                if instr.name == 'rz':   # as 'rz' gate take parameter theta
                    # print(f'instr rz {instr.params[0]}')
